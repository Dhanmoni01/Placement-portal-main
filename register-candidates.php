...
if (isset($_POST['email'])) {
  $email = $_POST['email'];
  if (strpos($email, 'nits.ac.in') === false) {
    $_SESSION['registerError'] = "Invalid email. Please use a nits.ac.in email address.";
          ...
          <div class="row latest-job margin-top-50 margin-bottom-20 bg-white">
            ...
            <form method="post" id="registerCandidates" action="adduser.php" enctype="multipart/form-data">
              <div class="col-md-6 latest-job ">
                ...
                <div class="form-group">
                  <input class="form-control input-lg" type="text" id="email" name="email" placeholder="Email *" required onchange="validateEmail()">
                  <span id="emailError" class="text-danger hide-me"></span>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <button class="btn btn-flat btn-success" name="register-button" id="register-button">Register</button>
                </div>
                <?php
...
function validateEmail() {
  ...
  // Validate email address locally before making an AJAX request
  const emailCheck = /.+nits.ac.in(?:\.[a-zA-Z]{2,}|$)/;
  const emailIsValid = emailCheck.test(email.trim().toLowerCase());

  if (!emailIsValid) {
    $('#emailError').text('Please use a nits.ac.in email address.');
    $('#emailError').show();
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.status === 'error') {
        ...
        $('#register-button').show(); // Show the register button if the email is valid
      }
    },
  ...
  // Hide the error if the email address is valid
  $('#emailError').text('');
  $('#emailError').hide();
  $('#register-button').show();
}

// Trigger the validation function on page load if an email address is already provided
$(function () {
  if ($('#email').val()) {
    validateEmail();
  }
